...
void CursorCoordclear()//유틸리티용 함수.커서를 0, 0으로 되돌림 
{
...
void typewriterEffect(const char* text, int delay) {//텍스트 출력
	// printf("╔ ╬ ═ ║ ╗ ╚ ╝");
	while (*text) {
		printf("%c", *text);
		fflush(stdout); // 화면에 즉시 출력
		Sleep(delay); // 문자 사이의 대기 시간 (밀리초 단위)
		text++;
	}
	printf("\n");

	//int delay = 100; // 0.1초 (100 밀리초) 대기
