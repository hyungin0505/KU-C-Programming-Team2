...
//
typedef struct Player_inform {
	int x;
	int y;
}Player_inform;//단순 x, y좌표만 쓰는 플레이어 inform 받기 위함. 
//
...
void PW_game();//비밀번호 게임. 
void PW_game_show_PW_screen();//힌트 + 패스워드 보여주는 화면
void PW_game_show_input_screen(int);//인풋화면
...
void Getdata_Minigame();//떨어지는 데이터를 성공적으로 줍는 게임
...
//박스게임용 구조체 정의.
typedef struct BoxController_map {
	char Map_inform[9][17];
	Player_inform PlayerCoord;
}BoxController_map;

//박스게임 함수
void BoxGame_init(int);
void BoxGame_Start();
void BoxGame_show_title_screen();
void BoxGame_win();
void BoxGame_defeat();
